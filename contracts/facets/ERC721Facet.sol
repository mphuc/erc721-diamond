...
contract ERC721 is Modifiers {
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        string memory _tokenURI = s.erc721TokenURIs[tokenId];
        ...
        virtual
    {
        ...
        s.erc721TokenURIs[tokenId] = _tokenURI;
    }
...
contract ERC721Facet is UsingEIP712, ERC721 {
        ...
        returns (uint256)
    {
        ...
        require(s.roles[MINTER_ROLE].members[signer], "Invalid signature");
        require(
            permission.currency != address(0) &&
                s.supportsCurrency[permission.currency],
        ...
        // EFFECTS
        uint256 before = s.erc20Pending[address(currency)][s.feeCollector];
        s.erc20Pending[address(currency)][s.feeCollector] = before.add(
            permission.mintingPrice
        );
        ...
        returns (uint256)
    {
        ...
        require(s.roles[MINTER_ROLE].members[signer], "Invalid signature");
        require(
            permission.currency == address(0) && s.supportsEther,
            "Currency not supported"
        );
        ...
        // EFFECTS
        uint256 before = s.ethPending[s.feeCollector];
        s.ethPending[s.feeCollector] = before.add(permission.mintingPrice);
        _safeMint(signer, permission.tokenId);
        _setTokenURI(permission.tokenId, permission.uri);
        _transfer(signer, permission.to, permission.tokenId);
